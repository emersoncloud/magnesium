...
export default function RouteFeed() {
  ...
  const sends = optimisticActivity.filter(
    (a) => a.action_type === "SEND" || a.action_type === "FLASH"
  );
  ...
  const myProposal = optimisticActivity.find(
      ...
      {user && (
        <div className="bg-yellow-50 border-l-4 border-yellow-400 p-4">
          <div className="flex items-center justify-between mb-2">
            ...
            {isSavingNote && (
              <span className="text-[10px] font-mono text-yellow-600">Saving...</span>
            )}
          </div>
          <textarea
            value={personalNote}
