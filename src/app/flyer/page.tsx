...
export default async function FlyerPage() {
  ...
  const [rawRoutes, activity] = await Promise.all([
    getRoutes(),
    getGlobalActivity(),
  ]);
          ...
          <div className="grid grid-cols-2 gap-8 items-center mt-4">
            ...
            <div className="flex flex-col gap-8 items-center">
              ...
              <Card className="w-full p-6 text-white border-2 border-rockmill bg-white shadow-[8px_8px_0px_0px_rgba(225,38,29,0.2)] transform -rotate-2">
                ...
                <div className="text-5xl font-black text-black mb-1">{gymStats.totalRoutes} <span className="text-black text-2xl font-bold">Routes</span></div>
                <div className="text-lg text-slate-600">Across {gymStats.uniqueGrades} difficulties</div>
              </Card>
            ...
            <div className="flex flex-col gap-4">
              ...
              {recentActivity.map((item) => (
                <Card key={item.id} className="p-4 flex items-center gap-4 border-2 border-slate-100 shadow-sm bg-white">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      {item.action_type === "SEND" && <Badge className="bg-green-500 hover:bg-green-600 text-[10px]">SENT</Badge>}
                      {item.action_type === "FLASH" && <Badge className="bg-yellow-400 hover:bg-yellow-500 text-black text-[10px]">FLASH</Badge>}
                      <span className="text-xs font-bold text-slate-700 truncate">{item.route_label || "Route"}</span>
                    </div>
                  </div>
                </Card>
              ))}
        ...
        <footer className="bg-rockmill p-8 flex items-center justify-between text-white relative overflow-hidden">
          ...
          <div className="relative z-10 bg-white p-2 rounded-lg shadow-lg transform -rotate-3">
            <div className="w-32 h-32 bg-white flex items-center justify-center border-2 border-slate-100">
              <span className="text-slate-300 font-mono text-xs text-center">QR CODE<br />HERE</span>
            </div>
            <div className="text-center mt-1">
              <span className="text-black font-black text-xs uppercase tracking-widest">Scan Me</span>
            </div>
          </div>
